...
def add_contact(*args):
    ...
    if notebook.add_record(rec):
        ...
        return f"{rec.name.value} contact is in the notebook."
...
def del_number(*args):
    ...
    if rec:
        ...
        return f"Phone {args[1]} has deleted successfully from contact {rec.name.value}."
    return f"Contact, with name {args[0]} not in notebook"
...
def del_contact(*args):
    rec = notebook.delete_record(args[0])
    if rec:
        return f'Contact {rec.name.value} delete successfully!'
    return f"Contact, with name {args[0]} not in notebook"
